...
((window, document, Math, undefined) => {
  ...
  function getColorFormatFromStr(str) {
    ...
    if (format === 'rgb' || format === 'hsl' || format === 'unit') 
    {
      return format;
    }
  ...
  function updateColor(rgba = {}, hsva = {}) {
    ...
    {
      case 'unit':
        colorValue.value = RGBAToUnitStr(currentColor);
      break;
      case 'hex':
  ...
  function RGBAToUnitStr(color)
  {
    return `R: ${Norm(color.r)} G: ${Norm(color.g)} B: ${Norm(color.b)} A: ${(color.a).toFixed(2)}`
  }

  function Norm(value)
  {
    return (value / 255.0).toFixed(2);
  }
  ...
  function ParseUnitColor(colorString) 
  {
  ...
  function init() {
    ...
    '<div id="clr-format" class="clr-format">'+
      '<fieldset class="clr-segmented">'+
        ...
        '<input id="clr-f0" type="radio" name="clr-format" value="unit">'+
        '<label for="clr-f0">Unit</label>'+
